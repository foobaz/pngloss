    ...
    for(int y = 0; y < height; y++) {
        for(int x = 0; x < width; x++) {
            ...
            for(int c = 0; c < bytesPerPixel; c++) {
                int offset = x*bytesPerPixel + c;
		int here = rows[y][offset];
                int errorHere = 0;
                if (here > 0 && here < 255) {
                    int up=0, left=0;
                    if (y > 0) {
                    ...
                    if (x > 0) {
                        left = rows[y][offset-bytesPerPixel];
                    }
...
void optimizeWithStride(
    ...
    optimizeWithRows(rows, width, height, bytesPerPixel, quantization);
    free(rows);
}
...
void optimizeForAverageFilter(
    ...
    optimizeWithStride(pixels, width, height, bytesPerPixel, stride, quantization);
}
